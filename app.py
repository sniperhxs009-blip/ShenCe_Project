...
3) 必须以 json 结构化对象输出（用于接口解析），仅包含字段：主要观点（3-5条要点列表）、资源关联（对象）、风险点（最多5条）、触发条件（最多5条）、建议（最多6条）、置信度（0-1）。
...
4) 严禁编造输入中没有的事实或数字。
严格以 json 结构返回，字段：分歧点（列表）、综合主要观点（列表）、情景区间（对象含 保守/基准/悲观）、综合资源关联（对象）、综合置信度、触发条件（列表）、建议（列表）。
"""
